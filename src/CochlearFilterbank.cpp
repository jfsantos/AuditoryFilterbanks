...
CochlearFilterbank::CochlearFilterbank(double fs, int num_channels,
                                       double low_freq) {
...
vector<vector<Biquad>> CochlearFilterbank::makeERBFilters(double fs,
                                                          int num_channels,
                                                          double low_freq) {
  double T = 1.0 / fs;
  ...
  for (int k = 0; k < num_channels; k++) {
    ...
    B2[k] = exp(-2 * B[k] * T);
    A11[k] = -(-B1[k] * T +
               2.0 * sqrt(3.0 + pow(2.0, 1.5)) * T *
                   sin(2.0 * cf[k] * M_PI * T) / exp(B[k] * T)) /
             2.0;
    A12[k] = -(-B1[k] * T -
               2.0 * sqrt(3.0 + pow(2.0, 1.5)) * T *
                   sin(2.0 * cf[k] * M_PI * T) / exp(B[k] * T)) /
    ...
    aux1 = complex<double>(-2) *
           exp(complex<double>(4) * i * cf[k] * M_PI * T) * T;
    aux2 = complex<double>(2) *
           exp(-(B[k] * T) + complex<double>(2) * i * cf[k] * M_PI * T) * T *
           (cos(2 * cf[k] * M_PI * T) -
            sqrt(3.0 - pow(2.0, 1.5)) * sin(2 * cf[k] * M_PI * T));
    aux3 = complex<double>(2) *
           exp(-(B[k] * T) + complex<double>(2) * i * cf[k] * M_PI * T) * T *
           (cos(2 * cf[k] * M_PI * T) +
            sqrt(3.0 - pow(2.0, 1.5)) * sin(2 * cf[k] * M_PI * T));
    aux4 = complex<double>(2) *
           exp(-(B[k] * T) + complex<double>(2) * i * cf[k] * M_PI * T) * T *
           (cos(2 * cf[k] * M_PI * T) -
            sqrt(3.0 + pow(2.0, 1.5)) * sin(2 * cf[k] * M_PI * T));
    aux5 = complex<double>(2) *
           exp(-(B[k] * T) + complex<double>(2) * i * cf[k] * M_PI * T) * T *
           (cos(2 * cf[k] * M_PI * T) +
            sqrt(3.0 + pow(2.0, 1.5)) * sin(2 * cf[k] * M_PI * T));
    aux6 = pow(-2 / exp(2 * B[k] * T) -
                   complex<double>(2) *
                       exp(complex<double>(4) * i * cf[k] * M_PI * T) +
                   complex<double>(2) *
                       (complex<double>(1) +
                        exp(complex<double>(4) * i * cf[k] * M_PI * T)) /
                       exp(B[k] * T),
               4);
    gain[k] = abs(abs((aux1 + aux2) * (aux1 + aux3) * (aux1 + aux4) *
                      (aux1 + aux5) / aux6));
...
Eigen::VectorXd CochlearFilterbank::ERBspace(double low_freq, double high_freq,
                                             int num_channels) {
  Eigen::VectorXd cf_array(num_channels);
  double aux = EAR_Q * MIN_BW;
  for (int i = 1; i <= num_channels; i++) {
    cf_array[i - 1] = -(aux) +
                      exp((i) * (-log(high_freq + aux) + log(low_freq + aux)) /
                          num_channels) *
                          (high_freq + aux);
  }
  return cf_array;
...
vector<Eigen::VectorXd> CochlearFilterbank::process(
    const Eigen::VectorXd &input, unsigned int n) {
  ...
  for (unsigned int ch = 0; ch < filters.size(); ch++) {
    ...
    futures.push_back(async(launch::async,
                            &CochlearFilterbank::process_channel_sample, this,
                            input, n, ch));
#else
...
Eigen::VectorXd CochlearFilterbank::process_channel(
    const Eigen::VectorXd &input, int n, int ch) {
...
Eigen::VectorXd CochlearFilterbank::process_channel_sample(
    const Eigen::VectorXd &input, unsigned int n, unsigned int ch) {

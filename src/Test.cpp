...
int main() {
  ...
  double t = tt.elapsed();
  cout << "Total processing time for the cochlear filterbank: " << t << " s."
       << endl;
#ifdef DEBUG
  for (int k = 0; k < 23; k++) {
    char fname[15];
    ...
    for (int n = 0; n < N_SAMPLES; n++) fprintf(f, "%e\n", y[k](n));
    fclose(f);
  }
#endif
  ...
  t = tt.elapsed();
  cout << "Total processing time for the modulation filterbank: " << t << " s."
       << endl;
#ifdef DEBUG
  for (int k = 0; k < 23; k++) {
    char fname[15];
